...
keeping its native {M.SOURCE_CRS} ({M.SOURCE_CRS_NAME}), and to PMTiles in
WGS84 for display. No features were added, removed or edited. The exact commands
are in [`tools/convert.py`]({M.CATALOG_REPO}/blob/main/tools/convert.py).

## Contributing

This catalog is generated, and both halves are open source: the catalog at
[`cholmes/portolan-catalog-trimet`]({M.CATALOG_REPO}) and the
[data browser]({M.BROWSER_URL}) at
[`cholmes/trimet-data-browser`]({M.BROWSER_REPO}). If something here is wrong or
could be better — a description, a column definition, a query, a style —
[open an issue]({M.CATALOG_REPO}/issues) or send a pull request. Corrections to
...
TriMet**. Conforms to the [Portolan](https://www.portolan-sdi.org/)
specification v0.1.0.

### Both halves are on GitHub, and contributions are welcome

Nothing here is hand-written — the catalog is generated, so a fix belongs in the
source rather than in the published files.

| | Repository | What lives there |
|---|---|---|
| **Catalog** | [`cholmes/portolan-catalog-trimet`]({M.CATALOG_REPO}) | The generators in [`tools/`]({M.CATALOG_REPO}/tree/main/tools) that build this catalog from TriMet's Shapefiles — the metadata, the column descriptions, the MapLibre styles and these docs. |
| **Data browser** | [`cholmes/trimet-data-browser`]({M.BROWSER_REPO}) | The [data browser]({M.BROWSER_URL}) itself — map preview, downloads, schema and license. |

**Found something wrong?** A description that misreads, a column definition that
does not match the data, a query that fails, a style that draws badly, a caveat
that is missing — please [open an issue]({M.CATALOG_REPO}/issues) or send a pull
request. Improvements are just as welcome as corrections.

Corrections to the *data itself* go to TriMet at
[gis@trimet.org](mailto:gis@trimet.org); this catalog only mirrors what they
publish and does not edit it.
...
[CLAUDE.md]({M.CATALOG_REPO}/blob/main/CLAUDE.md) says which file to edit for
...
The catalog is generated from [`cholmes/portolan-catalog-trimet`]({M.CATALOG_REPO})
and the [data browser]({M.BROWSER_URL}) from
[`cholmes/trimet-data-browser`]({M.BROWSER_REPO}). If you find an error in this
guide — a join that does not hold, a recipe that fails, a caveat that is missing
— [open an issue]({M.CATALOG_REPO}/issues) or send a pull request against the
