#!/usr/bin/env python3
"""The metadata gate: regenerating must reproduce the committed catalog exactly.
...
Not covered: the data files and thumbnails, which need GDAL, tippecanoe and
MapLibre Native. Those are verified by checksum in test_catalog.py instead.
...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if diffs:
            ...
            sys.exit(1)
        print(f"ok: {len(rels)} generated files reproduce byte-identically")
