...
![TriMet Transit Centers](./thumbnail.webp)
...
| Style | What it shows |
|---|---|
| [`default.json`](./styles/default.json) | The fifteen transit centers as prominent marks in TriMet brand orange. These are the network's timed-transfer hubs, so they are drawn larger than ordinary stops and stay visible at low zoom. |
| [`by-county.json`](./styles/by-county.json) | Colors the hubs by COUNTY. TriMet's district spans Multnomah, Washington and Clackamas counties, and this style shows how the transfer hubs are distributed across the three. |
| [`labeled.json`](./styles/labeled.json) | Transit center marks with NAME shown from zoom 9. Fifteen labels never collide, so this style works as a standalone overview of the network's hub structure. |
...
|---|---|---|
| [`transit-centers.parquet`](./transit-centers.parquet) | 6.4 KB | GeoParquet 1.1, 15 rows in 1 row group(s), zstd, Hilbert-ordered, bbox covering column |
| [`transit-centers.pmtiles`](./transit-centers.pmtiles) | 21.8 KB | Vector tiles for display, every feature kept at every zoom |
| [`thumbnail.webp`](./thumbnail.webp) | 47.3 KB | Rendered from `styles/default.json` over a light basemap |
